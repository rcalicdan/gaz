        ...
        <form wire:submit.prevent="save" class="p-6 space-y-8">
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <x-forms.field class="md:col-span-2" label="{{ __('Street Name') }}" name="registered_street_name" required>
                    ...
                    <x-forms.field label="{{ __('City') }}" name="registered_city" required>
                        <x-forms.input name="registered_city" wire:model="registered_city" placeholder="{{ __('Enter city') }}"
                            required />
                    </x-forms.field>

                    <x-forms.field label="{{ __('Zip Code') }}" name="registered_zip_code" required>
                        <x-forms.input name="registered_zip_code" wire:model="registered_zip_code" placeholder="{{ __('Enter zip code') }}"
                            required />
                    </x-forms.field>
            ...
            <div class="space-y-6">
                ...
                @else
                    <p class="text-sm text-gray-600 -mt-2 ml-14">{{ __('Physical location for waste pickup (will be geocoded for route optimization)') }}</p>

                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <x-forms.field class="md:col-span-2" label="{{ __('Street Name') }}" name="premises_street_name" required>
                        ...
                        <x-forms.field label="{{ __('City') }}" name="premises_city" required>
                            <x-forms.input name="premises_city" wire:model="premises_city" placeholder="{{ __('Enter city') }}"
                                required />
                        </x-forms.field>

                        <x-forms.field label="{{ __('Zip Code') }}" name="premises_zip_code" required>
                            <x-forms.input name="premises_zip_code" wire:model="premises_zip_code" placeholder="{{ __('Enter zip code') }}"
                                required />
                        </x-forms.field>
                            ...
                            <div class="flex gap-3 items-start bg-gray-50 p-4 rounded-lg border border-gray-200"
                                ...
                                <div class="flex items-center gap-2 pt-1">
                                    ...
                                    @if (count($phoneNumbers) > 1)
                                        <button type="button"
                                            wire:click="removePhoneNumber({{ $index }})"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-forms.field label="{{ __('Price Rate') }}" name="price_rate"
                        ...
                        <x-forms.input type="number" step="0.01" name="price_rate" wire:model="price_rate"
                            placeholder="{{ __('Enter price rate') }}" :icon="'<path stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'2\' d=\'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z\'></path>'" />
                    </x-forms.field>
                    ...
                    <x-forms.field label="{{ __('Tax Rate (%)') }}" name="tax_rate"
                        ...
                        <x-forms.input type="number" step="1" name="tax_rate" wire:model="tax_rate"
                            placeholder="{{ __('Enter tax rate') }}" :icon="'<path stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'2\' d=\'M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z\'></path>'" />
                    </x-forms.field>
