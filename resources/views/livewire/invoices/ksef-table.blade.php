<div class="flex flex-col min-h-full">
    <x-flash-session />
    <x-partials.dashboard.content-header :title="__('KSeF Invoices Monitoring')" />
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="relative">
                    <select wire:model.live="filterStatus"
                        ...
                        @foreach (\App\Enums\KsefStatus::cases() as $status)
                ...
                @php
                    $activeFiltersCount = collect([$filterStatus, $filterDateFrom, $filterDateTo])
                        ->filter()
                        ->count();
                @endphp
                ...
                <div class="flex flex-wrap gap-1.5 mt-2.5 pt-2.5 border-t border-gray-100">
                    @if ($filterStatus)
                        <span
                            ...
                            <span>📊</span> {{ \App\Enums\KsefStatus::from($filterStatus)->label() }}
                            <button wire:click="$set('filterStatus', '')" class="hover:text-red-900 ml-0.5">
                    ...
                    @if ($filterDateFrom)
                        <span
                            ...
                            <span>📅</span> {{ __('Od') }}: {{ $filterDateFrom }}
                            <button wire:click="$set('filterDateFrom', '')" class="hover:text-red-900 ml-0.5">
                                <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                    <path
                                        d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" />
                                </svg>
                    ...
                    @if ($filterDateTo)
                        <span
                            class="inline-flex items-center gap-1 px-2 py-0.5 bg-white text-emerald-700 rounded text-xs font-medium border border-emerald-200">
                            <span>📅</span> {{ __('Do') }}: {{ $filterDateTo }}
                            <button wire:click="$set('filterDateTo', '')" class="hover:text-red-900 ml-0.5">
                                <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                    <path
                                        d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" />
                                </svg>
