        ...
        <div class="flex space-x-1 bg-white p-1 rounded-lg border border-gray-200">
            <button wire:click="setTab('scheduled')"
                class="px-3 py-1 text-xs font-medium rounded-md transition-colors
                    {{ $activeTab === 'scheduled' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:text-gray-700' }}">
                {{ __('Scheduled') }}
            </button>
            <button wire:click="setTab('stale')"
                class="px-3 py-1 text-xs font-medium rounded-md transition-colors
                    {{ $activeTab === 'stale' ? 'bg-orange-100 text-orange-700' : 'text-gray-500 hover:text-gray-700' }}">
                {{ __('Needs Contact') }}
            </button>
            <button wire:click="setTab('expiring_declarations')"
                class="px-3 py-1 text-xs font-medium rounded-md transition-colors
                    {{ $activeTab === 'expiring_declarations' ? 'bg-red-100 text-red-700' : 'text-gray-500 hover:text-gray-700' }}">
                {{ __('Expiring Docs') }}
            </button>
        </div>
    </div>
        ...
        @if ($rows->isEmpty())
            <div class="text-center py-8">
                ...
                <p class="text-sm text-gray-500">
                    @if ($activeTab === 'expiring_declarations')
                        {{ __('No declarations expiring in the next 30 days.') }}
                    @else
                    ...
                    <li class="p-4 hover:bg-gray-50 transition-colors group">
                        <div class="flex items-start justify-between">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if ($activeTab === 'scheduled')
                                    <div class="flex items-center gap-2 mb-1">
                                        ...
                                        <a href="{{ route('clients.view', $row->client_id) }}"
                                    ...
                                    <div class="flex items-center gap-2 mb-1">
                                        <span class="text-xs font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded border border-orange-100">
                                            {{ $row->last_contact_date
                                                ? $row->last_contact_date->diffForHumans()
                                                : __('Never contacted') }}
                                        </span>
                                    </div>
                                    <a href="{{ route('clients.view', $row->id) }}"
                                        class="text-sm font-medium text-gray-900 hover:text-emerald-700 block mb-1">
                                        {{ $row->company_name }}
                                    </a>
                                    <p class="text-xs text-gray-500">{{ $row->city ?? __('No City') }}</p>

                                {{-- Expiring Declarations tab --}}
                                @elseif ($activeTab === 'expiring_declarations')
                                    @php $daysLeft = $row->declaration_days_until_expiry; @endphp
                                    <div class="flex items-center gap-2 mb-1">
                                    ...
                                    <p class="text-xs text-gray-500">
                                        ...
                                        <span class="font-medium">
                                            {{ $row->declaration_expiry_date->format('d.m.Y') }}
                                        </span>
                                    </p>
                                @endif
                            ...
                            <div class="ml-4 flex-shrink-0 flex items-center gap-2">
                                @if ($activeTab === 'scheduled')
                                    <button wire:click="completeReminder({{ $row->id }})"
                                        class="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-full transition-colors"
                                ...
                                @elseif ($activeTab === 'stale')
                                    <button wire:click="openReminderModal({{ $row->id }})"
                                        class="p-1.5 text-gray-400 hover:text-emerald-700 hover:bg-gray-50 rounded-full transition-colors"
                                        title="{{ __('Add Reminder') }}">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                    <div>
                        ...
                        <div class="mt-3 text-center sm:mt-5">
                            ...
                            <p class="text-sm text-gray-500 mt-2">
                                {{ __('When should we remind you to contact this client?') }}
                            </p>
                        </div>
                        ...
                        <div class="mt-4 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea wire:model="modalNote" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                    placeholder="{{ __('e.g. Discuss new pricing...') }}"></textarea>
                                @error('modalNote')
                                    <span class="text-red-500 text-xs">{{ $message }}</span>
                                @enderror
                            </div>
                    ...
                    <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                        <button type="button" wire:click="saveReminder"
                            class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-emerald-700 text-base font-medium text-white hover:bg-emerald-900 focus:outline-none sm:col-start-2 sm:text-sm">
                            {{ __('Set Reminder') }}
                        </button>
